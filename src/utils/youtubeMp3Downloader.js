...
const VIDEO_LENGTH_THRESHOLD = 60 * 10; // 10 minutes

export class YoutubeMp3Downloader extends EventEmitter {
  constructor(options) {
    super();
  ...
  async performDownload(task, callback) {
    ...
    try {
      ...
      return callback(err, resultObj);
    }

    if (
      info.player_response.videoDetails.lengthSeconds > VIDEO_LENGTH_THRESHOLD
    )
      return callback(new Error("Video is too long"), resultObj);
    ...
    stream.on("response", function (httpResponse) {
      ...
      const proc = new Ffmpeg({
        ...
        .on("error", function (err) {
          return callback(err.message, resultObj);
        })
        .on("end", function () {
